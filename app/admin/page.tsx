...
export default function AdminPage() {
  ...
  async function setResult(match:Match,hs:string,as:string){if(hs===""||as==="")return;const {error}=await supabase.from("matches").update({home_score:Number(hs),away_score:Number(as)}).eq("id",match.id);setStatus(error?"Не удалось сохранить фактический результат.":"Фактический результат сохранён. Очки пересчитаны.");await load();}
