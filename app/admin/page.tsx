...
export default function AdminPage() {
  ...
  const [roundName,setRoundName]=useState(""); const [home,setHome]=useState(""); const [away,setAway]=useState(""); const [kickoff,setKickoff]=useState(""); const [youtubeUrl,setYoutubeUrl]=useState(""); const [roundId,setRoundId]=useState(""); const [status,setStatus]=useState(""); const [editRound,setEditRound]=useState<string|null>(null); const [editMatch,setEditMatch]=useState<string|null>(null);
  ...
  async function setResult(match:Match,hs:string,as:string,youtube:string){if(hs===""||as==="")return;const {error}=await supabase.from("matches").update({home_score:Number(hs),away_score:Number(as),youtube_url:youtube.trim()||null,result_confirmed:true}).eq("id",match.id);setStatus(error?"Не удалось сохранить фактический результат.":"Фактический результат и обзор сохранены. Очки пересчитаны в обоих турнирах.");await load();}
  function startRoundEdit(r:Round){setEditRound(r.id);setRoundName(r.name);}
  function startMatchEdit(m:Match){setEditMatch(m.id);setRoundId(m.round_id);setHome(m.home_team);setAway(m.away_team);setKickoff(new Date(m.kickoff_at).toISOString().slice(0,16));setYoutubeUrl(m.youtube_url??"");}
  ...
  return <main className="page"><div className="container">
    ...
    <div className="grid" style={{gridTemplateColumns:"1fr 1fr",marginTop:20}}>
      ...
      <form className="card form" onSubmit={e=>{e.preventDefault();if(editMatch)updateMatch(editMatch);else addMatch(e)}}><h2>{editMatch?"Изменить матч":"Новый матч"}</h2><select className="input" value={roundId} onChange={e=>setRoundId(e.target.value)} required><option value="">Выберите тур</option>{rounds.map(r=><option key={r.id} value={r.id}>{r.name}</option>)}</select><input className="input" placeholder="Хозяева" value={home} onChange={e=>setHome(e.target.value)} required/><input className="input" placeholder="Гости" value={away} onChange={e=>setAway(e.target.value)} required/><input className="input" type="datetime-local" value={kickoff} onChange={e=>setKickoff(e.target.value)} required/><input className="input" type="url" placeholder="Ссылка на обзор YouTube (необязательно)" value={youtubeUrl} onChange={e=>setYoutubeUrl(e.target.value)}/><p className="badge" style={{marginTop:-4}}>Можно вставить обычную ссылку YouTube: youtube.com/watch?v=… или youtu.be/…</p><div style={{display:"flex",gap:8}}><button className="cta" type="submit" style={{border:0,cursor:"pointer",marginTop:0}}>{editMatch?"Сохранить":"Добавить матч"}</button>{editMatch&&<button type="button" className="badge" onClick={()=>{setEditMatch(null);setHome("");setAway("");setKickoff("");setYoutubeUrl("")}} style={{cursor:"pointer",background:"none"}}>Отмена</button>}</div></form>
    </div>
    {status&&<p className="badge" style={{marginTop:16}}>{status}</p>}

    <section className="card" style={{marginTop:20}}><h2>Матчи, прогнозы и фактические результаты</h2>{matches.length===0?<p className="badge">Матчей пока нет.</p>:matches.map(m=>{const round=rounds.find(r=>r.id===m.round_id);const sent=predictions.filter(p=>p.match_id===m.id&&p.submitted_at);return <div key={m.id} style={{padding:"16px 0",borderBottom:"1px solid var(--border)"}}><div className="badge">{round?.name??"Тур"} · {new Date(m.kickoff_at).toLocaleString("ru-RU")} · отправили {sent.length}</div><div style={{display:"flex",alignItems:"center",gap:8,marginTop:8,flexWrap:"wrap"}}><b>{m.home_team}</b><input className="input" style={{maxWidth:70}} type="number" min="0" defaultValue={m.home_score??""} id={`h-${m.id}`}/><span>:</span><input className="input" style={{maxWidth:70}} type="number" min="0" defaultValue={m.away_score??""} id={`a-${m.id}`}/><b>{m.away_team}</b><input className="input" style={{minWidth:260,flex:"1 1 260px"}} type="url" placeholder="Ссылка на обзор YouTube" defaultValue={m.youtube_url??""} id={`yt-${m.id}`}/><button className="cta" onClick={()=>{const h=(document.getElementById(`h-${m.id}`) as HTMLInputElement).value;const a=(document.getElementById(`a-${m.id}`) as HTMLInputElement).value;const yt=(document.getElementById(`yt-${m.id}`) as HTMLInputElement).value;setResult(m,h,a,yt)}} style={{border:0,cursor:"pointer",marginTop:0}}>{m.home_score!==null?"Сохранить факт":"Сохранить"}</button><button className="badge" onClick={()=>startMatchEdit(m)} style={{cursor:"pointer",background:"none"}}>Изменить матч</button><button className="badge" onClick={()=>deleteMatch(m.id)} style={{cursor:"pointer",background:"none"}}>Удалить</button></div>{m.youtube_url&&<div className="badge" style={{marginTop:8}}>🎬 Обзор YouTube добавлен</div>}
