...
import { MatchPreview, PreviousTeamStats } from "./match-preview";
...
export default function DashboardPage() {
  ...
  const [message, setMessage] = useState(""); const [status, setStatus] = useState(""); const [loadError, setLoadError] = useState(""); const [submittingRound, setSubmittingRound] = useState<string | null>(null); const [boardOpen, setBoardOpen] = useState(false); const [roundOpen, setRoundOpen] = useState<Record<string, boolean>>({}); const [boardRoundOpen, setBoardRoundOpen] = useState<Record<string, boolean>>({});
  ...
  function previousStats(team: string, currentRoundId: string): PreviousTeamStats | undefined {
    const currentOrder = rounds.find(r=>r.id===currentRoundId)?.sort_order ?? 0;
    const prior = matches
      .filter(m => (rounds.find(r=>r.id===m.round_id)?.sort_order ?? 0) < currentOrder && m.home_score !== null && m.away_score !== null && (m.home_team === team || m.away_team === team))
      .sort((a,b) => new Date(b.kickoff_at).getTime() - new Date(a.kickoff_at).getTime());
    if (!prior.length) return undefined;
    const last = prior[0];
    const home = last.home_team === team;
    const gf = home ? last.home_score! : last.away_score!;
    const ga = home ? last.away_score! : last.home_score!;
    const points = gf > ga ? 3 : gf === ga ? 1 : 0;
    const result = gf > ga ? "Победа" : gf === ga ? "Ничья" : "Поражение";
    const record = prior.slice().reverse().map(m => {
      const h = m.home_team === team; const f = h ? m.home_score! : m.away_score!; const a = h ? m.away_score! : m.home_score!;
      return f > a ? "В" : f === a ? "Н" : "П";
    }).join(" · ");
    return { result: `${gf}:${ga} — ${result}`, points, gf, ga, record };
  }
  ...
  return <main className="page"><div className="container">
    ...
    <div className="grid dashboard-grid">
      <section className="card"><h1>Мои прогнозы</h1><p className="badge intro">Заполни все матчи тура. До отправки твои прогнозы скрыты. После отправки открываются прогнозы всех участников, которые уже отправили этот же тур.</p>
        {rounds.length===0?<p className="badge">Туры пока не добавлены.</p>:rounds.map(round=>{const roundMatches=matches.filter(m=>m.round_id===round.id);const sent=submittedRound(round.id);const latestRoundId=rounds.reduce((latest,r)=>r.sort_order>latest.sort_order?r:latest,rounds[0]).id;const isOpen=roundOpen[round.id]??(round.id===latestRoundId);return <section key={round.id} className="card round-card">
          <button type="button" className="preview-toggle" onClick={()=>setRoundOpen(x=>({...x,[round.id]:!isOpen}))} aria-expanded={isOpen}><span><b>{round.name}</b> <span className="badge">{roundMatches.length} матчей</span></span><span className="round-toggle-right"><span className="badge">{sent?"✓ Прогнозы отправлены":`Отправили: ${boardCount(round.id)}/6`}</span><span>{isOpen?"▲":"▼"}</span></span></button>
          {isOpen&&<>
            {roundMatches.map(match=>{const locked=new Date(match.kickoff_at).getTime()<=Date.now();const value=predictions[match.id];const result=results[match.id];const isSubmitted=!!value?.submitted_at;const isSaved=!!savedDrafts[match.id];return <article key={match.id} className="match-card"><div className="badge">{formatDate(match.kickoff_at)} · {locked?"Приём закрыт":"Приём открыт"}</div><h3>{match.home_team} <span>—</span> {match.away_team}</h3>{match.home_score!==null&&match.away_score!==null&&<p className="badge">Фактический результат: {match.home_score}:{match.away_score}</p>}{isSubmitted?<div className="my-prediction">Мой прогноз: <strong>{value!.home_score}:{value!.away_score}</strong> <span className="badge">· отправлен</span>{result?.points!==null&&result?.points!==undefined?<strong> · {result.points} очк.</strong>:null}</div>:locked?<p className="badge">Прогноз не был отправлен вовремя.</p>:<div className="prediction-editor"><div className="score-box"><input className="score-input" type="text" inputMode="numeric" pattern="[0-9]*" maxLength={2} value={value?.home_score??""} placeholder="—" aria-label={`Счёт ${match.home_team}`} onChange={e=>changePrediction(match,"home_score",e.target.value)}/><span>:</span><input className="score-input" type="text" inputMode="numeric" pattern="[0-9]*" maxLength={2} value={value?.away_score??""} placeholder="—" aria-label={`Счёт ${match.away_team}`} onChange={e=>changePrediction(match,"away_score",e.target.value)}/></div><button className={`cta save-score${isSaved?" saved":""}`} onClick={()=>savePrediction(match)} disabled={isSaved}>{isSaved?"Сохранено":"Сохранить"}</button></div>}<MatchPreview home={match.home_team} away={match.away_team} previousHome={previousStats(match.home_team,match.round_id)} previousAway={previousStats(match.away_team,match.round_id)}/><MatchVideo url={match.youtube_url}/></article>})}
            {roundMatches.length>0&&!sent&&<button className="cta round-submit" disabled={submittingRound===round.id} onClick={()=>submitRound(round)}>{submittingRound===round.id?"Отправляем…":"Отправить прогноз за тур"}</button>}{sent&&<p className="badge round-note">✓ Тур отправлен. Ниже видны участники, которые тоже его отправили.</p>}
          </>}
        </section>})}{status&&<p className="status-line">{status}</p>}</section>
      <div className="dashboard-side">
        ...
        <section className="card board-card"><button type="button" className="board-toggle" onClick={()=>setBoardOpen(v=>!v)} aria-expanded={boardOpen}><span>📋 Общая таблица прогнозов</span><span className="board-chevron">{boardOpen?"▲":"▼"}</span></button>{boardOpen&&<><p className="badge">Видны только участники, которые отправили соответствующий тур.</p>{rounds.length===0?<p className="badge">Туры пока не добавлены.</p>:rounds.map(round=>{const roundMatches=matches.filter(m=>m.round_id===round.id);const roundBoardMatches=Array.from(new Map(board.filter(row=>row.round_id===round.id).map(row=>[row.match_id,row])).values());const latestRoundId=rounds.reduce((latest,r)=>r.sort_order>latest.sort_order?r:latest,rounds[0]).id;const isOpen=boardRoundOpen[round.id]??(round.id===latestRoundId);return <section key={round.id} className="card round-card board-round-card"><button type="button" className="preview-toggle" onClick={()=>setBoardRoundOpen(x=>({...x,[round.id]:!isOpen}))} aria-expanded={isOpen}><span><b>{round.name}</b> <span className="badge">{roundMatches.length} матчей</span></span><span className="round-toggle-right"><span className="badge">Отправили: {boardCount(round.id)}/6</span><span>{isOpen?"▲":"▼"}</span></span></button>{isOpen&&<div className="board-round-content">{roundBoardMatches.length===0?<p className="badge">Пока никто не отправил прогноз за этот тур.</p>:roundBoardMatches.map(match=>{const rows=board.filter(x=>x.match_id===match.match_id);return <div className="board-match" key={match.match_id}><div><b>{match.home_team} — {match.away_team}</b><div className="badge">{formatDate(match.kickoff_at)}{match.home_score!==null&&match.away_score!==null?` · Факт: ${match.home_score}:${match.away_score}`:""}</div></div><div className="board-rows">{rows.map(row=><div className="board-row" key={row.user_id}><span>{row.display_name}</span><strong>{row.predicted_home_score}:{row.predicted_away_score}</strong><span className="badge">{row.points===null?"—":`${row.points} очк.`}</span></div>)}</div></div>})}</div>}</section>})}</>}</section>
