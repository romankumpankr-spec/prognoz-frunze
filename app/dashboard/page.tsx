...
type Match = { id: string; home_team: string; away_team: string; kickoff_at: string; home_score: number | null; away_score: number | null; result_confirmed: boolean };
...
type BoardRow = { match_id: string; user_id: string; display_name: string; predicted_home_score: number; predicted_away_score: number; points: number | null; home_team: string; away_team: string; kickoff_at: string; home_score: number; away_score: number; round_id: string };
...
export default function DashboardPage() {
  ...
  async function load() {
    ...
    setUserId(user.id);
    const [profileQ, matchesQ, predictionsQ, resultsQ, chatQ, standingsQ, boardQ] = await Promise.all([
      supabase.from("profiles").select("display_name, role").eq("id", user.id).single(),
      supabase.from("matches").select("id, home_team, away_team, kickoff_at, home_score, away_score, result_confirmed").order("kickoff_at"),
      ...
      supabase.from("confirmed_prediction_board").select("match_id,user_id,display_name,predicted_home_score,predicted_away_score,points,home_team,away_team,kickoff_at,home_score,away_score,round_id").order("kickoff_at").order("display_name"),
    ]);
    const errors = [
      profileQ.error && `профиль: ${profileQ.error.message}`,
      matchesQ.error && `матчи: ${matchesQ.error.message}`,
      predictionsQ.error && `прогнозы: ${predictionsQ.error.message}`,
      resultsQ.error && `результаты: ${resultsQ.error.message}`,
      chatQ.error && `чат: ${chatQ.error.message}`,
      standingsQ.error && `таблица: ${standingsQ.error.message}`,
      boardQ.error && `общая таблица прогнозов: ${boardQ.error.message}`,
    ].filter(Boolean) as string[];
    setLoadError(errors.join(" | "));
    setProfile(profileQ.data ?? null); setMatches(matchesQ.data ?? []); setStandings(standingsQ.data ?? []); setBoard(boardQ.data ?? []);
  ...
  return <main className="page"><div className="container">
    ...
    <div className="grid" style={{gridTemplateColumns:"minmax(0,1.7fr) minmax(300px,1fr)"}}>
      <section className="card"><h1>Мои прогнозы</h1><p className="badge" style={{marginBottom:18}}>Прогноз можно менять до начала матча. Чужие прогнозы до подтверждения результата не показываются.</p>{matches.length===0?<p className="badge">Матчи пока не добавлены. Их добавит администратор.</p>:matches.map(match=>{const locked=new Date(match.kickoff_at).getTime()<=Date.now();const value=predictions[match.id];const result=results[match.id];return <article key={match.id} className="card" style={{marginBottom:12,background:"rgba(23,36,59,.65)"}}><div className="badge">{formatDate(match.kickoff_at)} · {locked?"Прогноз закрыт":"Приём прогнозов открыт"}</div><h3 style={{margin:"8px 0 12px"}}>{match.home_team} — {match.away_team}</h3>{match.home_score!==null&&match.away_score!==null&&<p className="badge">Результат: {match.home_score}:{match.away_score}{match.result_confirmed?" · подтверждён":" · ожидает подтверждения"}</p>}{locked&&value&&result?.points!==null&&<p style={{margin:"8px 0",fontWeight:800}}>Мой прогноз: {value.home_score}:{value.away_score} · <span>{result.points} {result.points===1?"очко":"очка"}</span></p>}{locked&&!value&&<p className="badge" style={{margin:"8px 0"}}>Прогноз не был сделан.</p>}{!locked&&<div style={{display:"flex",gap:8,alignItems:"center"}}><input className="input" style={{maxWidth:80}} type="number" min="0" value={value?.home_score ?? ""} placeholder="0" onChange={e=>setPredictions(x=>({...x,[match.id]:{match_id:match.id,home_score:Number(e.target.value),away_score:x[match.id]?.away_score ?? 0}}))}/><b>:</b><input className="input" style={{maxWidth:80}} type="number" min="0" value={value?.away_score ?? ""} placeholder="0" onChange={e=>setPredictions(x=>({...x,[match.id]:{match_id:match.id,home_score:x[match.id]?.home_score ?? 0,away_score:Number(e.target.value)}}))}/><button className="cta" onClick={()=>savePrediction(match)} style={{border:0,cursor:"pointer",marginTop:0}}>{value?"Сохранить изменения":"Сохранить прогноз"}</button></div>}</article>})}{status&&<p className="badge">{status}</p>}</section>
      <div style={{display:"grid",gap:16,alignContent:"start"}}>
        ...
        <section className="card"><h2 style={{marginTop:0}}>📋 Общая таблица прогнозов</h2><p className="badge">После подтверждения фактического результата здесь видно, кто какой счёт выбрал и сколько получил.</p>{board.length===0?<p className="badge">Пока нет подтверждённых матчей.</p>:boardMatches.map(match=>{const rows=board.filter(x=>x.match_id===match.match_id);return <div key={match.match_id} style={{marginTop:14,paddingTop:12,borderTop:"1px solid var(--border)"}}><div><b>{match.home_team} — {match.away_team}</b><div className="badge" style={{marginTop:4}}>Факт: {match.home_score}:{match.away_score} · {formatDate(match.kickoff_at)}</div></div><div style={{marginTop:8,display:"grid",gap:6}}>{rows.map(row=><div key={row.user_id} style={{display:"grid",gridTemplateColumns:"1fr auto auto",gap:8,alignItems:"center",padding:"6px 0"}}><span>{row.display_name}</span><b>{row.predicted_home_score}:{row.predicted_away_score}</b><span className="badge">{row.points ?? 0} очк.</span></div>)}</div></div>})}</section>
