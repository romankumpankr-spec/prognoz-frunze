...
export default function DashboardPage() {
  ...
  async function load() {
    ...
    const map: Record<string, Prediction> = {}; for (const x of predictionsQ.data ?? []) map[x.match_id] = { match_id:x.match_id, home_score:String(x.home_score), away_score:String(x.away_score), submitted_at:x.submitted_at }; setPredictions(map);
  ...
  async function savePrediction(match: Match) {
    ...
    setStatus(error?`Не удалось сохранить прогноз: ${error.message}`:"Прогноз сохранён как черновик."); setTimeout(()=>setStatus(""),2500); if(!error) await load();
  }
