...
const TEAMS = [
  "АЕК Афины", "Астон Вилла", "Арсенал", "Атлетико", "Барселона", "Бавария", "Бетис", "Боруссия Дортмунд", "Брюгге", "Будё-Глимт", "Викинг", "Вильярреал", "Галатасарай", "Интер", "Комо", "Ланс", "ЛАСК", "Лейпциг", "Лилль", "Ливерпуль", "МЮ", "Ман Сити", "Наполи", "Порту", "ПСВ", "ПСЖ", "Реал", "Рома", "Сабах", "Славия", "Слован", "Спортинг", "Стuttgart", "Фейеноорд", "Фенербахче", "Шахтер"
];

export default function TeamsPage() {
  const supabase = createClient(); const router = useRouter();
  const [userId, setUserId] = useState(""); const [profile, setProfile] = useState<Profile | null>(null);
  ...
  async function load() {
    ...
    setUserId(user.id);
    const [{ data: p }, { data: mine }, { data: subs }, { data: all }] = await Promise.all([
    ...
    setProfile(p ?? null); setUserId(user.id); setSelected((mine ?? []).map(x => x.team_name)); setSubmitted(subs ?? []); setSelections(all ?? []);
  }
  useEffect(() => { load(); }, []);

  const submittedSet = useMemo(() => new Set(submitted.map(x => x.user_id)), [submitted]);
  const submittedCount = submitted.filter(x => x.user_id !== userId).length;
  const people = useMemo(() => {
    const map = new Map<string, string>();
    for (const x of selections) if (submittedSet.has(x.user_id)) map.set(x.user_id, x.user_id);
    return Array.from(map.keys());
  }, [selections, submittedSet]);
  ...
  return <main className="page"><div className="container">
    ...
    <section className="card team-hero"><span className="eyebrow">Отдельный турнир</span><h1>🏆 Выбери 6 команд</h1><p>Выбери ровно шесть команд из списка. Сейчас мы только фиксируем выбор — подсчёт очков и итоговая таблица будут добавлены позже.</p><div className="team-counter"><strong>{selected.length}/6</strong><span>{submittedSet.has(userId) ? "Выбор отправлен" : selected.length === 6 ? "Можно отправлять" : "Нужно выбрать ещё " + (6-selected.length)}</span></div></section>
    ...
    <section className="card" style={{marginTop:16}}><h2>📋 Таблица выбора команд</h2><p className="badge">Здесь отображаются только участники, которые уже отправили свои 6 команд.</p>{people.length===0?<p className="badge">Пока никто не отправил выбор.</p>:<div className="draft-table">{people.map(id=>{const picks=selections.filter(x=>x.user_id===id).map(x=>x.team_name); return <div className="draft-row" key={id}><b>{id===userId?profile?.display_name??"Вы":"Участник"}</b><span>{picks.join(" · ")}</span></div>})}</div>}</section>
  </div></main>;
}
